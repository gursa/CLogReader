...
/*!
    ...
    Затем вызывается GetNextLine, который возвращает следующую строку удовлетворяющцю шаблону.
    \code
    CLogReader lr;
    bool rc = lr.Open("test.log");
    rc = lr.SetFilter("some*order");
    if(rc)
    {
        char buf[4096];
...
private:
    ...
    //! Объект класса, занимающегося проецированием текстового файла в память
    CFileMapper m_fileMapper;

    //! Реализация регэкспов
    CRegex m_regex;
};
